...
// 	int nErr = setsockopt(m_Socket, IPPROTO_TCP, TCP_NODELAY, &chOpt, sizeof(char));
	...
	if (setsockopt(m_Socket, SOL_SOCKET, SO_KEEPALIVE, (char *)&iOptVal, iOptLen) == 0)
	{
		// 设置超时详细信息
		tcp_keepalive	klive;
		klive.onoff = 1; // 启用保活
...
bool CClientSocket::ConnectProxyServer(LPCSTR lpszHost, UINT nPort)
{
		...
		if(m_proxyans1->Method == 2 && strlen(m_strUserName) > 0)
		{
			...
			strcpy_s(m_authreq.NamePass, m_strUserName);
			memcpy(m_authreq.NamePass + nUserLen, &nPassLen, sizeof(int));
			strncpy_s(m_authreq.NamePass + nUserLen + 1,sizeof(m_authreq.NamePass), m_strPassWord,strlen(m_strPassWord));
										 ...
										 UINT nProxyPort /*= 1080*/, LPCSTR lpszUserName /*= NULL*/, LPCSTR lpszPassWord /*= NULL*/ )
{
	...
	if (lpszProxyHost != NULL)
		strcpy_s(m_strProxyHost, sizeof(m_strProxyHost),lpszProxyHost);

	m_nProxyPort = nProxyPort;
	if (lpszUserName != NULL)
		strcpy_s(m_strUserName, sizeof(m_strUserName),lpszUserName);
	if (lpszPassWord != NULL)
		strcpy_s(m_strPassWord, sizeof(m_strPassWord),lpszPassWord);
}
