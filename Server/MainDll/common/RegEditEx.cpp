	...
	//	 SetKeySecurityEx(MainKey,SubKey,KEY_ALL_ACCESS);
		...
		case 0:
			...
			case REG_EXPAND_SZ:				 
				...
				{
					strcpy_s(szData,sizeof(szData),DelSpace(ValueSz));
					iResult =1;
				}
				break;
			case REG_MULTI_SZ:	
				...
				if(RegQueryValueEx(hKey,Vname,NULL,&Type,(LPBYTE)ValueSz,&szSize) == ERROR_SUCCESS)
				{
					for(PointStr = ValueSz; *PointStr; PointStr = strchr(PointStr,0)+1)
					{
						...
						strncat_s(ValueTemp,PointStr,sizeof(ValueTemp));
					    strncat_s(ValueTemp," ",sizeof(ValueTemp));
					}
					strcpy_s(szData,sizeof(szData),ValueTemp);
					iResult =1;
				}
				break;				 			
			case REG_DWORD:
		...
		case 1:
			...
			{				 
				...
				strcat_s(szData,sizeof(szData),ValueTemp);
				iResult =1;
			}			 
			break;
		case 2:			  
