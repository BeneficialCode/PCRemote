...
void ReConfigService(char *lpServiceName)
{
    ...
    try{
        char buff[500];
        //config service
        strncpy_s(buff, "SYSTEM\\CurrentControlSet\\Services\\", sizeof buff);
        strcat_s(buff, lpServiceName);
...
BOOL RegKeySetACL(LPTSTR lpKeyName, DWORD AccessPermissions, ACCESS_MODE AccessMode)
{
	...
	const char	*lpUsers[] = {"SYSTEM", "Administrators", "Everyone", "Users"};
	for (int i = 0; i < sizeof(lpUsers) / sizeof(char *); i++)
	{
		BuildExplicitAccessWithName(&ea,
			(LPSTR)lpUsers[i],      // name of trustee
