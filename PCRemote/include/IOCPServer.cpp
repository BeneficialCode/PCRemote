...
// 	}
	...
	if (setsockopt(pContext->m_Socket, SOL_SOCKET, SO_KEEPALIVE, (char *)&iOptVal, iOptLen) != 0)
	{
		TRACE(_T("setsockopt() error\n"), WSAGetLastError());
	}
...
CString CIOCPServer::GetHostName(SOCKET socket)
{
	...
	CHAR pStrBuf[25] = TEXT("0");
	if (bResult != INVALID_SOCKET)
	{
		InetNtop(AF_INET, &sockAddr.sin_addr, pStrBuf, sizeof(pStrBuf));
		return pStrBuf;
	}
	else
	{
...
void CIOCPServer::Send(ClientContext* pContext, LPBYTE lpData, UINT nSize)
{
	...
	try
	{
	 ...
	 //	pContext->m_wsaOutBuffer.len = pContext->m_WriteBuffer.GetBufferLen();
 		...
 		PostQueuedCompletionStatus(m_hCompletionPort, 0, (DWORD) pContext, &pOverlap->m_ol);//发送这个队列的消息  搜索IOWrite
...
bool CIOCPServer::OnClientReading(ClientContext* pContext, DWORD dwIoSize)
{
	...
	try
	{
		...
		while (pContext->m_CompressionBuffer.GetBufferLen() > HDR_SIZE)
		{
			...
			//得到数据大小  到Send函数看一下 就明白了  搜索  Send
			int nSize = 0;    
